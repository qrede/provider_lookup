defmodule ProviderLookupWeb.QueryFilterEx do
  ...
  def filter(query, changeset, filters) do
    ...
    filters |> Enum.reduce(query, create_where_clauses_reducer(changes))
  end

  defp create_where_clauses_reducer(changes) do
    fn %{name: name, field_name: field_name, binding: binding, method: method}, acc ->
      case Map.fetch(changes, name) do
        {:ok, value} ->
          acc |> create_where_clause(field_name, binding, method, value)
  ...
  defp create_where_clause(acc, field_name, binding, method, value) do
    case method do
      :eq ->
        acc
        |> where(
